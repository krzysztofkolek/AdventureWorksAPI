namespace AdventureWorks.Repository.Test.Repository.Production
{
    using EntityClasses.Production;
    using Main;
    using NUnit.Framework;

    [TestFixture]
    [Category("Production")]
    public class LocationRepositoryTest : BaseRepositoryTest<Location, LocationRepositoryTest>
    {
        public override System.Linq.Expressions.Expression<System.Func<Location, bool>> SearchForExpresion()
        {
            throw new System.NotImplementedException();
        }

        public override void BaseSearchForAsserts(System.Collections.Generic.IList<Location> input)
        {
            throw new System.NotImplementedException();
        }

        [Test]
        [Order(1)]
        public void LocationRepositoryTest_Insert()
        {
            BaseInsert();
        }

        [Test]
        [Order(2)]
        public void LocationRepositoryTest_GetById()
        {
            BaseGetById();
        }

        [Test]
        [Order(3)]
        public void LocationRepositoryTest_SearchFor()
        {
            BaseSearchFor();
        }

        [Test]
        [Order(4)]
        public void LocationRepositoryTest_GetAll()
        {
            BaseGetAll();
        }

        [Test]
        [Order(5)]
        public void LocationRepositoryTest_Delete()
        {
            BaseDelete();
        }
    }
}